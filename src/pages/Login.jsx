  ...
  function iniciarSesion(user,password){
    if(user==="admin"&&password==="contraseña"){
      ...
      Swal.fire({
        icon: "error",
        title: "Oops...",
        text: "Te equivacaste en el usuario o contraseña",
        footer: '<p style=color:orange>Intenta nuevamente</p>'
      });
    }
  }
  return (
    <div className="card">
        ...
        <form className="form">
          ...
          <input spellCheck="false" className="input" type="email" name="email" id="email-input" onChange={(e)=>(setEmail(e.target.value))} />
          <div className="frg_pss">
            <label className="label_input" htmlFor="password-input">Password</label>
            <a href>Forgot password?</a>
          </div>
          <input spellCheck="false" className="input" type="text" name="password" id="password-input" onChange={(e)=>(setPassword(e.target.value))}  />
          <button onClick={()=>iniciarSesion(getEmail,getPassword)} className="submit" type="button" to={"/home"} >Submit</button>
        </form>
